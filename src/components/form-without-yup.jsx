...
const FormWithoutYup= () => {
    ...
    const isValidEmail = (email) =>{
        const emaliRegex = /^\S+@\S+\.\S+$/;
        return emaliRegex.test(email);
    }; //for basic email validation

    const isValidPhoneNumber = (phoneNumber)=>{
        const phoneRegex = /^\d{10}$/;
        return phoneRegex.test(phoneNumber);
    }; //expression for basic phone number validation
    ...
    const validateForm = ()=>{
        ...
        if(!formData.password){
            ...
            newErrors.password = "Password must be at least 8 characters long and contaion at least one symbol, one number";
        }
        if(!formData.confirmPassword){
            newErrors.confirmPassword = "Confirm passwors is required";
